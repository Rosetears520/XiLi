...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="熵权 TOPSIS（按年份）")
    parser.add_argument(
        "--input-file",
        default=None,
        help="输入 Excel 文件路径",
    ...
    parser.add_argument(
        "--output-file",
        default=None,
        help="输出 Excel 文件路径（默认: topsis_by_year.xlsx）",
    )
    parser.add_argument(
        "--year-col",
        default="年份",
...
def cli(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        output_file = prepare_output_path(args.output_file, "topsis_by_year.xlsx")
